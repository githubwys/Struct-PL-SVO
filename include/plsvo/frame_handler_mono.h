...
protected:
  ...
  initialization::KltHomographyInit klt_homography_init_; //前两个关键帧使用单应矩阵，估计位姿//!< Used to estimate pose of the first two keyframes by estimating a homography.
  DepthFilter* depth_filter_;                   //深度估计算法在并行线程运行，并且初始化3D点//!< Depth estimation algorithm runs in a parallel thread and is used to initialize new 3D points.
  cv::Mat debug_img;
