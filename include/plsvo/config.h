...
public:
  ...
  /// Has points
  static bool& hasPoints() { return getInstance().has_pt; }

  /// Has line segments
  static bool& hasLines() { return getInstance().has_ls; }

  /// Employ points in initialization
  static bool& initPoints() { return getInstance().init_pt; }

  /// Employ line segments in initialization
  static bool& initLines() { return getInstance().init_ls; }
  ...
  /// Number of pyramid levels used for features (line segments).
  static size_t& nPyrLevelsSegs() { return getInstance().n_pyr_levels_segs; }
  ...
  /// Feature grid size of a cell in [px] (line segments).
  static size_t& gridSizeSegs() { return getInstance().grid_size_segs; }
  ...
  /// Number of iterations in local bundle adjustment.
  static size_t& poseOptimNumIterRef() { return getInstance().poseoptim_num_iter_ref; }
  ...
  /// Maximum number of points to optimize at every iteration.
  static size_t& structureOptimMaxSegs() { return getInstance().structureoptim_max_segs; }
  ...
  /// Number of iterations in structure optimization (lines).
  static size_t& structureOptimNumIterSegs() { return getInstance().structureoptim_num_iter_segs; }
  ...
  /// Minimum translation distance between two keyframes. Relative to the average height in the map.
  static double& kfSelectMinDistT() { return getInstance().kfselect_mindist_t; }

  /// Minimum rotation distance between two keyframes. Relative to the average height in the map.
  static double& kfSelectMinDistR() { return getInstance().kfselect_mindist_r; }
  ...
  /// Select only line segments with a minimum length
  static double& lsdMinLength() { return getInstance().lsd_min_length; }
  ...
  /// Maximum number of features that should be tracked.
  static size_t& maxFtsSegs() { return getInstance().max_fts_segs; }
  ...
  /// If the number of tracked features drops below this threshold. Tracking quality is bad. (line segments)
  static size_t& qualityMinFtsSegs() { return getInstance().quality_min_fts_segs; }

  /// If within one frame, this amount of features are dropped. Tracking quality is bad.
  static double& qualityMaxFtsDrop() { return getInstance().quality_max_drop_fts; }

  /// If within one frame, this amount of features are dropped. Tracking quality is bad.
  static double& qualityMaxFtsDropSegs() { return getInstance().quality_max_drop_fts_segs; }
...
private:
  ...
  string trace_dir;
  size_t n_pyr_levels;
  size_t n_pyr_levels_segs;           // always n_pyr_levels > n_pyr_levels_segs  --> TODO: check matcher.cpp
  bool use_imu;
  size_t core_n_kfs;
  double map_scale;
  size_t grid_size;
  size_t grid_size_segs;
  ...
  size_t poseoptim_num_iter_ref;
  size_t structureoptim_max_pts;
  size_t structureoptim_num_iter;
  size_t structureoptim_max_segs;
  size_t structureoptim_num_iter_segs;
  double loba_thresh;
  double loba_robust_huber_width;
  size_t loba_num_iter;
  double kfselect_mindist_t;
  double kfselect_mindist_r;
  double triang_min_corner_score;
  double lsd_min_length;            // TODO: change for some heuristic (the longest N lines or sth like that)
  size_t triang_half_patch_size;
  size_t subpix_n_iter;
  size_t max_n_kfs;
  double img_imu_delay;
  size_t max_fts;
  size_t max_fts_segs;              // pendiente
  size_t quality_min_fts;
  size_t quality_min_fts_segs;      //pendiente
  double quality_max_drop_fts;
  double quality_max_drop_fts_segs;    //pendiente
};
